...
io.on('connection', (socket) => {
    socket.on('mouse', (data) => {
      fs.writeFileSync('../../voice/yourname.txt', data);
      var readMe = fs.readFileSync('../../voice/state.txt','utf8');
      if (readMe === "0"){
        fs.writeFileSync('../../voice/state.txt', "1");
      }
      else{
        fs.writeFileSync('../../voice/state.txt', "0");
      }
    });

    socket.on('cancel', () => {
      fs.writeFileSync('../../voice/state.txt', "0");
      fs.writeFileSync('../../voice/state2.txt', "1");
    });
  });
